...
class ImageSelectionApp:
    ...
    def preprocess_image(self, image, region_coords):
        # 선택한 영역의 크기에 따라 이미지를 처리
        x0, y0, x1, y1 = region_coords
        selected_region = image.crop((x0, y0, x1, y1))

        # 선택한 이미지를 흰색 배경에 arial 폰트로 변환된 텍스트 이미지가 될 수 있도록 처리
        new_image = Image.new('L', selected_region.size, 'white')
        ...
        # 이미지를 리사이즈하지 않고 중앙에 배치
        new_image.paste(selected_region, (0, 0))

        # 이미지를 numpy 배열로 변환 및 정규화
        processed_image = np.array(new_image) / 255.0
    ...
    def reconstruct_image(self):
        ...
        if selected_region is not None:
            processed_image = self.preprocess_image(self.image, region_coords)
